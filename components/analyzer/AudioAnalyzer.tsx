...
export const AudioAnalyzer = memo(function AudioAnalyzerComponent() {
  ...
  return (
    <div ref={rootCallbackRef} className="flex flex-col h-screen bg-background">
      ...
      {/* Tiny info bar — branding + version at very bottom */}
      <div className="flex-shrink-0 flex items-center justify-center gap-2 py-0.5 bg-card/60 border-t border-border/30" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        <span className="font-mono text-[9px] font-bold tracking-[0.2em] text-muted-foreground/40 uppercase">DoneWell Audio Analyzer</span>
        <span className="font-mono text-[9px] tracking-[0.1em] text-muted-foreground/25 tabular-nums">v{process.env.NEXT_PUBLIC_APP_VERSION ?? '0.0.0'}</span>
      </div>
    </div>
  )
})
        ...
        <FullscreenPortalGate rootEl={rootEl}>
          ...
          {workerError && (
            <div role="alert" className={`px-3 py-1.5 sm:px-4 sm:py-2 border-b ${
              dspWorker.isPermanentlyDead
                ? 'bg-red-500/10 border-red-500/30'
                : 'bg-amber-500/5 border-amber-500/20'
            }`}>
              <div className="flex items-center gap-2.5">
                <AlertTriangle className={`w-3.5 h-3.5 flex-shrink-0 ${dspWorker.isPermanentlyDead ? 'text-red-500' : 'text-amber-500'}`} />
                <p className={`text-sm font-mono ${dspWorker.isPermanentlyDead ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'}`}>
                  {dspWorker.isPermanentlyDead
